...
/**
 * Unit test for {@link StarttlsCommand}.
 */
public class StarttlsCommandTest {
    ...
    @BeforeClass
    public void setUp() {
        final Class<?> classUnderTest = StarttlsCommand.class;
    ...
    public void testGetCommandLine() throws SmtpAsyncClientException, IllegalAccessException {
        final SmtpRequest cmd = new StarttlsCommand();
        Assert.assertEquals(cmd.getCommandLineBytes().toString(StandardCharsets.US_ASCII), "STARTTLS\r\n", "Expected results mismatched");
    ...
    public void testGetCommandType() {
        Assert.assertSame(new StarttlsCommand().getCommandType(), SmtpRFCSupportedCommandType.STARTTLS, "Incorrect command type");
    }
    ...
    public void testGetDebugData() {
        Assert.assertEquals(new StarttlsCommand().getDebugData(), "");
    }
}
